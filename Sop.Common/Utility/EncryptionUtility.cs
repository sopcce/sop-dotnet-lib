...
//--------------------------------------------------------------
//<version>V0.5</verion>
//<createdate>2019-6-6</createdate>
//<author>guojq</author>
//<email>sopcce@qq.com</email>
//<log date="2019-6-6" version="0.5">创建</log>
//--------------------------------------------------------------
//<sopcce.com>
...
#pragma warning restore 618
        ...
        public static string XxTeaDecrypt(string data, string key)
        {
            try
            {
                ...
                {
                    v[0] = ConvertHexStringToUint(data.Substring(i, 8));
                    v[1] = ConvertHexStringToUint(data.Substring(i + 8, 8));
                    decode(v, k);
                    int num4 = 0;
                    do
                    {
        ...
        public static uint[] FormatKey(string key)
        {
            try
            {
                ...
                {
                    numArray2[index] = ConvertStringToUint(key.Substring(i, 4));
                    index++;
                }
                return numArray2;
            }
        ...
        #region xxtea private
        private static uint ConvertHexStringToUint(string input)
        {
            try
            {
                byte[] buffer = new byte[5];
        ...
        private static string ConvertUintToHexString(uint input)
        {
            try
            {
                string str2 = "";
        ...
        private static void code(uint[] v, uint[] k)
        {
            ...
            while (num5-- != 0)
            {
                num += ((((num2 << 4) ^ (num2 >> 5)) + num2) ^ (num3 + k[num3 & 3]));
                num3 += num4;
                num2 += ((((num << 4) ^ (num >> 5)) + num) ^ (num3 + k[(num3 >> 11) & 3]));
            }
            v[0] = num;
            v[1] = num2;
        }
        ...
        private static void decode(uint[] v, uint[] k)
        {
            ...
            while (num4-- != 0)
            {
                num2 -= ((((num << 4) ^ (num >> 5)) + num) ^ (num5 + k[(num5 >> 11) & 3]));
                num5 -= num3;
                num -= ((((num2 << 4) ^ (num2 >> 5)) + num2) ^ (num5 + k[num5 & 3]));
            }
            v[0] = num;
            v[1] = num2;
        }
